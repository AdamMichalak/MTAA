...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def checkValidity(self, uri):
        ...
        if validity > now:
            ...
            del registrar[uri]
            logging.warning("registracia na %s uplynula" % uri)
            return False
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 V poriadku, OK")
                return
        else:
            now = int(time.time())
            validity = now + expires

        logging.info("Sprava z: %s - Kontakt: %s" % (fromm, contact))
    ...
    def processInvite(self):
        logging.debug("-----------------")
        logging.debug(" POZVANKA prijata ")
        logging.debug("-----------------")
        origin = self.getOrigin()
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Nespravny Request")
            return
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                if data[0] == 'SIP/2.0 100 Trying':
                    history.write(f'{currTime}::Trying Call {data[2]} {data[3]}\n')
                    logging.info("<<< %s" % 'SIP/2.0 100 Pokus o nadviazanie')
                elif data[0] == 'SIP/2.0 180 Ringing':
                    history.write(f'{currTime}::Ringing {data[2]} {data[3]}\n')
                    logging.info("<<< %s" % 'SIP/2.0 180 Zvoni')
                elif data[0] == 'SIP/2.0 487 Request terminated':
                    history.write(f'{currTime}::Call terminated {data[2]} {data[3]}\n\n')
                    logging.info("<<< %s" % 'SIP/2.0 487 Ziadost ukoncena')
                elif data[0] == 'SIP/2.0 486 Busy here':
                    history.write(f'{currTime}::Call terminated, line is busy - {data[2]} {data[3]}\n\n')
                    logging.info("<<< %s" % 'SIP/2.0 486 Linka obsadena')
                elif data[0] == 'SIP/2.0 603 Decline':
                    history.write(f'{currTime}::Call declined {data[2]} {data[3]}\n\n')
                    logging.info("<<< %s" % data[0])
                elif data[0] == 'SIP/2.0 200 Ok':
                    if 'INVITE' in data[5]:
                        history.write(f'{currTime}::Call accepted {data[2]} {data[3]}\n')
                    elif 'BYE' in data[5]:
                        history.write(f'{currTime}::Call ended {data[2]} {data[3]}\n\n')
                    logging.info("<<< %s" % data[0])
                else:
                    logging.info("<<< %s" % data[0])
                logging.debug("---\n<< server odoslal [%d]:\n%s\n---" % (len(text), text))
    ...
    def handle(self):
        ...
        if rx_request_uri.search(request_uri) or rx_code.search(request_uri):
            ...
            logging.debug("---\n>> server prijal [%d]:\n%s\n---" % (len(data), data))
            logging.debug("Prijate z %s:%d" % self.client_address)
            self.processRequest()
        else:
            if len(data) > 4:
                showtime()
